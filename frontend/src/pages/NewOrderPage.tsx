...
import { Camera, CheckCircle2, FileImage, FileSearch, ShieldCheck, TriangleAlert, UploadCloud } from 'lucide-react';
...
import type { ServiceOrder, Vehicle } from '@/types/app';
...
export default function NewOrderPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      setOrder(parsed);
      toast.success('OS lida com sucesso. Agora revise os itens e anexe as fotos obrigatórias.');
    } catch (error) {
          ...
          <CardContent className="space-y-5">
            {!order ? (
              <div className="rounded-3xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
                Após o upload, o sistema exibirá os itens lidos e exigirá a foto comprobatória por item antes do envio ao gestor.
              </div>
            ) : (
              <>
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <div className={`rounded-3xl border p-5 ${evidenceComplete ? 'border-success/30 bg-success/10' : 'border-warning/30 bg-warning/10'}`}>
                    <div className="mb-2 flex items-center gap-2">
                      {evidenceComplete ? <CheckCircle2 className="h-4 w-4 text-success" /> : <TriangleAlert className="h-4 w-4 text-warning" />}
                      <span className="text-xs uppercase tracking-[0.18em]">{evidenceComplete ? 'Pronto para envio' : 'Ação necessária'}</span>
                    </div>
                    <p className="font-medium">
                      {evidenceComplete
                        ? 'Todos os itens possuem foto comprobatória.'
                        : 'Ainda faltam fotos obrigatórias para liberar a aprovação.'}
                    </p>
                  </div>
                </div>
                ...
                <div className="rounded-3xl border border-border/70 bg-secondary/50 p-5">
                  ...
                  <p className="text-sm text-muted-foreground">
                    O envio para aprovação do gestor só será liberado quando todos os itens tiverem a foto comprobatória exigida.
                  </p>
                </div>

                <Button onClick={confirmOrder} disabled={!evidenceComplete} className="w-full rounded-2xl">
